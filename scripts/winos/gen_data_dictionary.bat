...
REM command line options of SchemaSpy
echo Generating dictionary...
echo ### Generating dictionary... >> %DICT_LOG% 2>&1
%schema_spy% -t pgsql -host %host% -db %dbname% -u %username% -p %pword% -o "%doc_path%" -dp %common_dir%postgresql-9.1-901.jdbc4.jar -schemas "address,administrative,application,bulk_operation,cadastre,document,party,source,system,transaction,public" -gv %graph_vis% -hq -I "(.*_historic)" -X "(.*.rowidentifier)|(.*.rowversion)|(.*.change_action)|(.*.change_user)|(.*.change_time)" -ahic -norows -noimplied -meta "%common_dir%\metafiles" >> %DICT_LOG% 2>&1
